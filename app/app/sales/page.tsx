...
import { addSaleItem, createSale, listProducts, listSales } from '@/lib/db'
        ...
        <div className="lg:col-span-2 bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
          <div className="p-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
            <div className="relative flex-1 max-w-xs">
              ...
              <input type="text" placeholder="Filtrar vendas..." className="w-full pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-xs outline-none" />
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-50">
                {sales.length === 0 ? (
                  <tr>
                    <td colSpan={3} className="px-6 py-10 text-sm text-slate-400 text-center">Nenhuma venda registada.</td>
                  </tr>
                ) : null}
                {sales.map((sale: any) => (
